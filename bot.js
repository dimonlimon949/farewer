  ...
  for (const cmdDir of cmdDirs) {
    ...
    for (const file of files) {
      if (file.endsWith('.js')) {
        ...
        command.forEach(cmd => {
          cmd.push(path.join(cmdDir, file)); 
        });
          ...
          for (let i = 0; i < win.length; i++) {
            ...
            if (dbUser) {
              ...
              if (userWon) {
                ...
                text += `✅ [id${user.id2}|${dbUser.tag}] ставка ${utils.sp(user.amount)} на x${user.type} выиграла! 🏆 (приз ${utils.sp(roundedWinnings)} ${val4}) 💰🎉\n`;
...
vk.updates.on('chat_invite_user', async (message, bot) => {
  ...
  const isInviterCreator = spolerIds.includes(Math.abs(inviterId));
...
async function sendLog(logData) {
  try {
      ...
      console.log('Invite Log:', logData);
      
      // Отправка в чат логов
      const logMessage = [
          `📌 ${logData.action.toUpperCase()} [${logData.status}]`,
          `👤 Пригласивший: @id${logData.inviter.id} ${logData.inviter.isCreator ? '(создатель)' : ''}`,
          `🎯 Приглашенный: ${logData.invited.type === 'community' ? '@club' : '@id'}${Math.abs(logData.invited.id)}`,
...
const lastMessageText = new Map(); // Map для хранения текста последнего сообщения каждого пользователя
...
loadCommands().then(() => {
  ...
  // Получаем текст предыдущего сообщения пользователя
  ...
  if (previousMessageText === message.text && lastMessageTime.has(userId) && (now - lastMessageTime.get(userId)) < cooldownTime) {
...
if (/\[@club${groupId}|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[@club${groupId}|(.*)\]/ig, '').trim();
    ...
    if (!double.find(x => x.id === message.senderId) && !message.isChat) {
      ...
      if (message.referralSource && message.referralValue) {
        ...
        let id = Number(message.referralSource);

        let user = await double.find((x) => x.id === id);
          ...
          if (multiply === 2) {
            ...
            if (user.notifications)
              await vk.api.messages.send({
                user_id: user.id,
                message: `УВЕДОМЛЕНИЕ ✅
        ▶ Вам начислено 10.000.000 ${val1} за то что игрок указал, что вы его пригласили!
        🔕 Введите «Уведомления выкл», если не хотите получать подобные сообщения`,
        keyboard: JSON.stringify({
          "inline": true,
          "buttons": [
          ...
          if (multiply === 2) {
            ...
            if (user.notifications)
              await vk.api.messages.send({
                user_id: user.id,
                message: `УВЕДОМЛЕНИЕ ✅
        ▶ Вам начислено 20.000.000 ${val1} за то что игрок указал, что вы его пригласили!
        🔕 Введите «Уведомления выкл», если не хотите получать подобные сообщения`,
        keyboard: JSON.stringify({
          "inline": true,
          "buttons": [
          ...
          if (multiply === 2) {
            ...
            if (user.notifications)
              await vk.api.messages.send({
                user_id: user.id,
                message: `УВЕДОМЛЕНИЕ ✅
        ▶ Вам начислено 50.000.000 ${val1} за то что игрок указал, что вы его пригласили!
        🔕 Введите «Уведомления выкл», если не хотите получать подобные сообщения`,
        keyboard: JSON.stringify({
          "inline": true,
          "buttons": [
      ...
      keyboard: JSON.stringify({
        ...
        await message.send(
          `✅ Вы успешно активировали приглашение! Начислен бонус в виде 2 донат-кейсов! 🎁✨`
        );
    ...
    const bot = async (text, params = {}) => {
      if (!message || !message.send) {
          console.error("Ошибка: Отсутствует message или message.send");
          return 'Ошибка: не удалось отправить сообщение';
      }
  
      // Определяем ID пользователя или ставим значение по умолчанию
      const userId = message.user ? message.user.uid : 'unknown'; //  Используем 'unknown' для неизвестных пользователей
         let userMention = "";
      let messageText1 = text
  
      if (message.user) {
        userMention = message.user.mention ? `@id${message.user.id} (${message.user.tag})` : message.user.tag;
      } else {
        userMention = "От неизвестного пользователя"
      }
      let messageText = userMention;
      ...
      try {
           otvet = messageText; // Присваиваем значение messageText переменной otvet
  
          console.log(`Отправляю сообщение ботом (ID: ${userId}): ${otvet}`); // Используем otvet в console.log
  
          await message.send(messageText, params); // Отправляем сообщение
          return otvet;  // возвращаем otvet
      } catch (error) {
          console.error("Ошибка при отправке сообщения:", error);
          const errorMessage = `Ошибка при отправке сообщения ботом (ID: ${userId}): ${errorMessage}`;
           otvet = errorMessage;
          console.log(`Ошибка при отправке сообщения ботом (ID: ${userId}): ${errorMessage}`); // Добавлено логирование ошибки
          return errorMessage;
      }
  };
  ...
  let botResponseText = null; // Инициализируем переменную ответа бота
  ...
  const command = commands.find(x => {
    ...
    if (double.find(x => x.id == message.senderId) && command ) {
    if (message.user.limsms >= 500 && !message.user.settings.topdon){
      return bot(`Лимит команд исчерпан! ⏱️ Возвращайтесь завтра.

Снять лимиты: Topdon (150р) через репорт! 💸`);
    }
  }
...
if (!groupInfo || groupInfo.length === 0) {
    ...
    const commentText = message.text; // Предполагаем, что текст комментария доступен в поле text
...
setInterval(() => {
  double
    ...
    .forEach((x) => {
      ...
      caseDescription = `🎁 ВАУ! Вы получили ${caseType} кейс за TOPDON! 🎉 Энергия увеличена на +5! ⚡`;
