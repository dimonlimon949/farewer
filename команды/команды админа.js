...
cmd.hear(/^(?:выдать редкий|редкий выдать|редкий)\s+(\d+)\s+(\d+)$/i, async (message, bot) => {
  ...
  double = JSON.parse(fs.readFileSync('./database/users.json', 'utf8')); // Обновляем double после записи
...
cmd.hear(/^(?:легу выдать|лег|легендарку|выдать легу|легу|лега)\s+(\d+)\s+(\d+)$/i, async (message, bot) => {
  if (message.user.settings.adm < 2) return bot(`доступно администратору 2 уровня и выше`);
