...
cmd.hear(/^(?:репорт)\s(.+)$/i, async (message, bot) => {
  ...
  try {
      ...
      if (message.attachments.length > 0) {
          for (const attachment of message.attachments) {  // Используем for...of
              let attachmentString = '';
              switch (attachment.type) {
                  case 'video':
                      attachmentString = `video${attachment.ownerId}_${attachment.id}_${attachment.accessKey || ''}`;
                      break;
                  case 'photo':
                      attachmentString = `photo${attachment.ownerId}_${attachment.id}_${attachment.accessKey || ''}`;
                      break;
                  case 'audio':
                      attachmentString = `audio${attachment.ownerId}_${attachment.id}_${attachment.accessKey || ''}`;
                      break;
                  case 'doc':
                      attachmentString = `doc${attachment.ownerId}_${attachment.id}_${attachment.accessKey || ''}`;
                      break;
                  case 'wall':
                      attachmentString = `wall${attachment.ownerId}_${attachment.id}_${attachment.accessKey || ''}`;
                      break;
                  case 'audio_message':
                      attachmentString = `audio_message${attachment.ownerId}_${attachment.id}_${attachment.accessKey || ''}`;
                      break;
                  default:
              ...
              if (attachmentString) {
                  attachmentIds.push(attachmentString);
              }
          }
      } else {
          // Если нет вложений, добавляем video-168874636_456244881
          attachmentIds.push("video-168874636_456244881");
      }

      // === Отправка репорта ===
      const keyboard = {  // Объявляем keyboard вне send, для читаемости
          inline: true,
          buttons: [
              [
                  {
...
@all 🎅`,
          attachment: attachmentIds.join(','), // Добавляем attachments
          keyboard: JSON.stringify(keyboard), // Передаем объект keyboard
    ...
    if (message.args[2].toLowerCase() === "положительно") {
      ...
      ratingMessage = `▶️ Игрок @id${message.user.id} (${message.user.tag}) оценил Ваш ответ на положительно! 👍${smileng}\n` +
        `〽️ Ваша репутация: ${utils.sp(user.astats.norm)} 👍 | ${utils.sp(user.astats.bad)} 👎`,
              { attachment: `photo-203302510_4572391557` };
    }
