  ...
  if (!message.isChat || message.chat.type === 1) {
    ...
    if (message.isChat) {
      ...
      if (chatss) {
        ...
        if (chatss.statuepeoplelvl >= 6 && chatss.statuemoneylvl >= 3 && chatss.statuemsglvl >= 3) {

                   if (chatss.gift > Date.now()) {
            const timeLeft = chatss.gift - Date.now();
            const minutes = Math.floor(timeLeft / 60000); // Минуты
            const seconds = Math.floor((timeLeft % 60000) / 1000); // Секунды

            return bot(`Подарок можно получить раз в 15 минут, пожалуйста подождите 🎈 Осталось: ${minutes} м. ${seconds} с.`);
          }

          chatss.gift = Date.now() + 900000;
          ...
          if (rand == 1) {
            ...
            message.send({ sticker_id: 108216 });
          ...
          if (rand == 2) {
            ...
            message.send({ sticker_id: 108217 });
          ...
          if (rand == 3) {
            ...
            message.send({ sticker_id: 79405 });

          }

          if (rand == 4) {

            let rands = utils.random(1, 1000)

            message.user.btc += rands;

            bot(`Вы получили подарок статуи! ✅\n\n🎊Ваш подарок: ${utils.sp(rands)} BTC 🌐`);

            message.send({ sticker_id: 79406 });
          ...
          if (rand == 5) {
            ...
            bot(`Вы получили подарок статуи! ✅\n\n🎊 Ваш подарок: ${rands} ${val2} 💰`);

            message.send({ sticker_id: 108210 });

          }

          if (rand == 6) {

            let rands = utils.random(1, 500)
