...
cmd.hear(/^(?:отмена|cancel|отменить|отмени|верни)\s(5)$/i, async (message, bot) => {
    if (message.chat.type === 2) {
        ...
        message.chat.games[message.chat.games.length - 1].stavki = message.chat.games[message.chat.games.length - 1].stavki.filter(x => x.id !== message.senderId || x.type !== 5);

        return bot(`Ваша ставка на x5 в размере ${utils.sp(lastBet.amount)} ${val4} была отменена. Вам возвращено ${utils.sp(refundAmount)} ${val4}.`);
    }
});
