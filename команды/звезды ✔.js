  ...
  if (!message.isChat || message.chat.type === 1) {
    if (!message.args[1]) {
      return bot(`Звезды: 
        ...
        ${message.user.stars5 ? '✅️' : '❌'} 5. Донатный гигант - 15.000 ЧакоРуб\nПрибыль: 30 ЧакоРуб/час.
    ...
    const starsData = [
      { id: 1, name: 'Солнце', cost: 15000, resource: 'almaz', profit: 100, emoji: '🔅' },
      { id: 2, name: 'Сириус', cost: 10000, resource: 'materia', profit: 75, emoji: '🌠' },
      { id: 3, name: 'Красный гигант', cost: 5000, resource: 'obsidian', profit: 50, emoji: '🔴' },
      { id: 4, name: 'Плазмовый гигант', cost: 3000, resource: 'plazma', profit: 10, emoji: '🌌' },
      { id: 5, name: 'Донатный гигант', cost: 15000, resource: 'rub', profit: 30, emoji: '💵' }
    ];
    ...
    if (message.user.ruds[star.resource] >= star.cost) {
      ...
      return bot(`Необходимо ${utils.sp(star.cost)} ${star.resource === 'almaz' ? 'алмазов' : star.resource === 'materia' ? 'материи' : 'обсидиана'}`);
    }
  }
});
